...
/***/ function(module, exports, __webpack_require__) {
	...
	document.title = window.app.lang['main.title'];
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Story = __webpack_require__(194);

	var defers = {
	  help: $.get('/public/views/help.html')
	};
	...
	var ractive = new Ractive({
	  ...
	  data: {
	    ...
	    lang: function lang(id) {
	      return window.app.lang[id];
	    }
	  }
	});

	// return active map {1: true, 10: true..}
	// from current (section) bubble up to root
	function resolveActive(section, sections) {
	  if (!section) return [0];
	  var parent = sections.filter(function (e) {
	    return e.id === section.parent;
	  })[0];
	  return [section.id].concat(resolveActive(parent, sections));
	}

	ractive.on('section.click', function (e, section) {
	  ractive.fire('section.change', section);
	});

	ractive.on('section.change', function (section) {
	  var current = ractive.get('section');
	  var toggle = ractive.get('toggle.section');

	  if (current !== section) {
	    ractive.set('toggle.section', true);
	    ractive.set('section', section);

	    var ids = resolveActive(section, ractive.get('sections'));
	    var activeMap = _.object(ids, ids.map(function () {
	      return true;
	    }));
	    ractive.set('menu.active', activeMap);
	  } else {
	    if (section.content) {
	      ractive.set('toggle.section', !toggle);
	    }
	  }
	});

	ractive.on('narration.close', function () {
	  ractive.set('toggle.lock', false);
	  ractive.set('toggle.section', false);
	});

	ractive.on('lock.click', function () {
	  ractive.toggle('toggle.lock');
	});

	ractive.on('help.click', function () {
	  defers.help.then(function (tmpl) {
	    vex.open({
	      content: tmpl,
	      css: {
	        'font-size': '12px'
	...
	// content pane is clicked, close the narration
	window.addEventListener('message', function (evt) {
	  var action = evt.data.action;
	  if (action === 'contentwindow.click' && !ractive.get('toggle.lock')) {
	    ractive.set('toggle.section', false);
	  }
	}, false);

	// handle section jumper
	window.addEventListener('message', function (evt) {
	  var action = evt.data.action;
	  var sections = ractive.get('sections');
	  if (action === 'jump.section') {
	    var section = sections.filter(function (e) {
	      return e.id === evt.data.sectionId;
	    })[0];
	    if (!section) return;
	    ractive.fire('section.click', null, section);
	  }
	}, false);

	// bugfix: hover on col-section and col-narration will trigger
	// hover popup to close
	['section', 'narration'].forEach(function (cls) {
	  $('.col.col-' + cls).on('mouseenter', function () {
	    window.postMessage({
	      action: 'area.hover',
	      toggle: false
	    }, document.URL);
...
/***/ function(module, exports) {
	...
	function load(id) {
	  var d = $.Deferred();
	  $.getJSON('/stories/' + id + '/story.json').then(function (data) {
	    var defaultSectionId = data.defaultSectionId;
	    var sections = data.sections;
	    injectNumChild(sections);
	    var tree = resolveTree(sections);
	...
	function injectNumChild(sections) {
	  var res = {};
	  sections.forEach(function (section) {
	    var id = section.parent;
	    if (!id) return;
	    if (!(id in res)) {
	      res[id] = 0;
	    }
	  ...
	  sections.forEach(function (section) {
	    section.numChild = res[section.id] || 0;
	  });
	}

	// initial tree menu
	function resolveTree(sections) {
	  var buildNode = function buildNode(section) {
	    return { section: section, childs: [] };
	  };
	  var nodes = _.object(_.pluck(sections, 'id'), sections.map(function (e) {
	    return buildNode(e);
	  }));
	  var tree = { level: 0, section: { id: 0 }, childs: [] };

	  sections.forEach(function (section) {
	    var parent = section.parent === null ? tree : nodes[section.parent];
	    var node = nodes[section.id];
	    parent.childs.push(node);
	  });

	  // iterate to inject level
	  var walk = function walk(node, level) {
	    node.level = level;
	    for (var i in node.childs) {
	      walk(node.childs[i], level + 1);
	    }
	  };
	  walk(tree, 0);
	  return tree;
...
/***/ function(module, exports, __webpack_require__) {
	...
	var ractive = __webpack_require__(193);

	var tid = null,
	    delay = 500;

	ractive.observe('section', function (section) {
	  if (!section) return;

	  window.clearTimeout(tid);

	  ractive.set('toggle.narration', false);

	  // do not open narration pane when content == null
	  if (section.content === null) {
	    ractive.fire('narration.close');
	    return;
	  }

	  tid = window.setTimeout(function () {
	    ractive.set('show.section', section);
	    ractive.set('toggle.narration', true);
	  }, delay);
	});
...
/***/ function(module, exports, __webpack_require__) {
	...
	function isExternal(section) {
	  return _.startsWith(section.url, 'http://');
	}
	...
	function setFrameUrl(url) {
	  var $wrapper = $('#frame_content').html('');
	  var $frame = $('<iframe>', {
	    src: url,
	    'class': 'full-wh',
	    frameborder: 0
	...
	ractive.observe('section', function (section) {
	  ...
	  window.clearTimeout(tid);
	  var url = frameUrl(section);

	  if (isExternal(section)) {
	    setFrameUrl(url);
	    ractive.set('toggle.coverFade', true);
	    ractive.set('toggle.cover', false);
	    return;
	  }

	  ractive.set('toggle.cover', true);
	  tid = window.setTimeout(function () {
	    ractive.set('toggle.coverFade', false);
	    tid = window.setTimeout(function () {
	      setFrameUrl(url);
	    }, delay);
	  }, 100);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _ = __webpack_require__(198),
	    ractive = __webpack_require__(193),
	    Story = __webpack_require__(194);

	var state = {
	  section: null
	};

	function initialSection(story) {
	  var sections = story.sections;
	  var id = parseInt(window.location.hash.substring(1));
	  if (!_.isFinite(id)) {
	    id = story.defaultSectionId;
	  }
	...
	// load the story
	$.getJSON('/public/data/stories.json').then(function (stories) {
	  var storyId = window.location.pathname.split('/')[2];
	  Story.load(storyId).then(function (story) {
	    ractive.set('sections', story.sections);
	    ractive.set('tree', story.tree);

	    // default section
	    var section = initialSection(story);
	    ractive.fire('section.click', null, section);
	  });
	});
	...
	window.addEventListener('popstate', function (event) {
	  try {
	    (function () {
	      var id = parseInt(window.location.hash.substring(1));
	      var section = ractive.get('sections').filter(function (e) {
	        return e.id === id;
	      })[0];
	      if (section) {
	        ractive.fire('section.change', section);
...
/***/ function(module, exports, __webpack_require__) {
	...
	// load languages
	$.getJSON('/language').then(function (config) {
	  ractive.set({
	    select: {
	      languages: config.languages,
	      language: config.current
	    }
	  });

	  ractive.observe('select.language', function (id) {
	    if (id !== config.current) {
	      window.location.href = resolveUrl(id);
	    }
	  });
	});

	function resolveUrl(langId) {
	  return '//' + resolveHost(langId) + window.location.pathname + window.location.hash;
	}

	function resolveHost(langId) {
...
/***/ function(module, exports) {
	...
	var dLang = $.getJSON('/public/lang/datatables.json');

	var datatable = null;

	function open(opts) {
	  vex.open({ content: $content, className: 'vex vex-theme-default popup-map' });
	  ...
	  datatable = $content.find('.table-map').html('').DataTable({
	    scrollY: '400px',
	    scrollColapse: true,
	    paging: false,
	    data: opts.rows,
	...
	window.addEventListener('message', function (evt) {
	  var action = evt.data.action;
	  if (action === 'popup.map.show') {
	    dLang.then(function (lang) {
	      evt.data.language = lang;
	      open(evt.data);
	    });
	  }
	}, false);

	module.exports = { ractive: ractive, $content: $content };
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Chart = __webpack_require__(203);

	var $content = $('<div></div>');

	var ractive = new Ractive({
	  el: $content.get(0),
	  template: '#template_popup_area',
	  data: {
	    area: {},
	    years: [],
	    indicator: null,
	    priority: null,
	    table: {},
	    value: function value(indicator, year) {
	      var table = ractive.get('table');
	      var key = indicator + '.' + year;
	      if (key in table && table[key] !== null) {
	        return numeral(table[key]).format('0,0.00');
	      }
	      return '';
	    },
	...
	function open(_ref) {
	  ...
	  var _parse = parse({ rows: rows, indicators: indicators, indicator: indicator });

	  var years = _parse.years;
	  var headers = _parse.headers;
	  var table = _parse.table;

	  var area = rows[0];
	  ractive.set({ years: years, headers: headers, table: table, indicator: indicator, priority: priority, area: area });

	  vex.open({
	    content: $content,
	    className: 'vex vex-theme-default popup-area'
	  });

	  var indicatorById = function indicatorById(id) {
	    return indicators.filter(function (e) {
	      return e.id === id;
	    })[0];
	  };
	  var rowsByIndicatorId = function rowsByIndicatorId(id) {
	    return rows.filter(function (e) {
	      return e.indicatorId === id;
	    });
	  };

	  Chart({
	    el: $content.find('.chart-priority'),
	    indicator: indicatorById('PRIO'),
	    rows: rowsByIndicatorId('PRIO'),
	    opts: {
	      chart: {
	        type: 'column'
	      }
	  ...
	  var povChart = Chart({
	    el: $content.find('.chart-poverty'),
	    indicator: indicatorById('POV'),
	    rows: rowsByIndicatorId('POV'),
	    opts: {
	      chart: {
	        type: 'line'
	      }
	  ...
	  injectPovChart(povChart, area);
	}

	// FIXME: using ugly injection
	function injectPovChart(chart, area) {
	  var lang = window.app.lang;
	  chart.series[0].name = sprintf(lang['static.povertyrate'], { name: area.district });
	  chart.addSeries({
	    name: lang['static.nationpovertyrate'],
	    data: [['2005', 16.66], ['2009', 15.42], ['2015', 10.96]],
	    color: '#CC4444',
	    animation: {
	      duration: 300
	    },
	...
	function parse(_ref2) {
	  ...
	  var headers = indicators.sort(function (a, b) {
	    if (a.id === indicator.id) return -1;
	    if (b.id === indicator.id) return 1;
	    return 0;
	  });

	  var years = _.chain(rows).pluck('year').uniq().sort().value();
	  var table = {};
	  for (var i in rows) {
	    var r = rows[i];
	    table[r.indicatorId + '.' + r.year] = r.value;
	  }
	...
	window.addEventListener('message', function (evt) {
	  if (evt.data.action === 'popup.area.main') {
	    open(evt.data);
	  }
	}, false);
...
/***/ function(module, exports) {
	...
	function Chart(_ref) {
	  ...
	  var values = rows.map(function (r) {
	    var category = '' + r.year;
	    var value = r.value == null ? null : Math.round(r.value * 100) / 100;
	    return [category, value];
	  });
	  var categories = _.chain(years).map(function (e) {
	    return [e, e];
	  }).object().value();
	  ...
	  if (indicator.range && indicator.range.colors) {
	    colorByPoint = true;
	    colors = resolveColors({ years: years, values: values, indicator: indicator });
	  }

	  var range = resolveRange(indicator);
	  ...
	  var extended = $.extend(true, {
	    ...
	    yAxis: {
	      ...
	      min: range.min,
	      max: range.max,
	      tickInterval: indicator.range.step || undefined
	    },
	    plotOptions: {
	      column: {
	        dataLabels: {
	  ...
	  return $(el).highcharts(extended).highcharts();
	}

	function resolveColors(_ref2) {
	  var years = _ref2.years;
	  var values = _ref2.values;
	  var indicator = _ref2.indicator;

	  var colorMap = indicator.range.colors;
	  var map = {};
	  for (var i in values) {
	    var year = values[i][0];
...
/***/ function(module, exports, __webpack_require__) {
	...
	var positioning = __webpack_require__(205),
	    Chart = __webpack_require__(203);

	var $content = $('<div class="popup-hover"></div>').css({ 'display': 'none' }).appendTo(document.body);

	var ractive = new Ractive({
	  el: $content.get(0),
	  template: '#template_popup_hover',
	  data: {
	    indicator: null,
	    rows: [],
	    location: {},
	...
	function open(opts) {
	  ...
	  var pos = positioning({
	    x: opts.position.x + sectionWidth,
	    y: opts.position.y + topPad,
	    width: $content.width(),
	...
	// automatic close om mouse enter
	...
	window.addEventListener('message', function (evt) {
	  var action = evt.data.action;
	  if (action === 'area.hover') {
	    evt.data.toggle ? open(evt.data) : close();
	  }
	}, false);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var ractiveRoot = __webpack_require__(193);

	var $content = $('<div class="popup-legend"></div>').css({ 'display': 'none' }).appendTo(document.body);
	...
	// close if section changed
	ractiveRoot.observe('section', close);

	window.addEventListener('message', function (evt) {
	  var action = evt.data.action;
	  if (action === 'popup.legend.show') {
	    var data = evt.data;
	    $content.css({
	      display: evt.data.toggle ? 'block' : 'none'
	    });
	    if (data.toggle) {
	      open(data);
	    }
...
/***/ function(module, exports) {
	...
	window.addEventListener('message', function (evt) {
	  if (evt.data.action === 'popup.error') {
	    vex.open({
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.push([module.id, "\nbody, html {\n    width: 100%;\n    height: 100%;\n}\n\nbody {\n    font-family: 'Open Sans', sans-serif;\n}\n\n.header-menu {\n    background: #2A93FC;\n    color: #FFF;\n    box-shadow: 0px 1px 3px rgba(0, 0, 0, 0.2);\n    position: fixed;\n    top: 0;\n    left: 0;\n    width: 100%;\n    z-index: 10;\n    height: 80px;\n    padding: 0 20px;\n    font-family: verdana, sans-serif;\n    line-height: 150%;\n    font-size: 15px;\n}\n\n.header-menu strong {\n    font-size: 17px;\n}\n\n.logo-container {\n    height: 80px;\n    line-height: 80px;\n    position: absolute;\n    right: 20px;\n    top: 6px;\n    z-index: 11;\n}\n\n.logo-container img {\n    padding-left: 10px;\n    position: relative;\n    height: 68px;\n}\n\n.lang-container {\n    line-height: 80px;\n    float: left;\n}\n\n#ractive_root {\n    width: 100%;\n    height: 100%;\n}\n\n\n.col-wrapper {\n    width: 100%;\n    height: 100%;\n    position: relative;\n    overflow: hidden;\n}\n\n.col {\n    position: absolute;\n    overflow: auto;\n    top: 0;\n    bottom: 0;\n}\n\n.col-inner {\n    padding-top: 80px;\n}\n\n.col-section {\n    z-index: 4;\n    left: 0;\n    background: #EEE;\n    width: 250px;\n    font-size: 85%;\n    box-shadow: 2px 2px 2px 1px rgba(0, 0, 0, 0.2);\n}\n\n.col-narration {\n    z-index: 3;\n    left: 250px;\n    width: 400px;\n    background: #FFF;\n    padding: 15px;\n    -webkit-transition: all 0.8s ease;\n            transition: all 0.8s ease;\n    box-shadow: 2px 2px 2px 1px rgba(0, 0, 0, 0.2);\n    font-size: 85%;\n}\n\n.col-narration h2 {\n    font-family: 'Voltaire', sans-serif;\n    padding-bottom: 10px;\n    border-bottom: 1px dashed #CCC;\n    font-size: 25px;\n    font-weight: normal;\n}\n\n.col-narration .button-container {\n    text-align: right;\n}\n\n.col-narration .button-container > * {\n    vertical-align: middle;\n    display: inline-block;\n}\n\n.col-narration .narration {\n    -webkit-transition: all 0.5s ease;\n            transition: all 0.5s ease;\n}\n\n.col-narration .narration.fade {\n    opacity: 0;\n}\n\n.icon-lock {\n    height: 25px;\n    margin-right: 5px;\n    opacity: 0.3;\n    cursor: pointer;\n}\n\n.icon-lock.active {\n    opacity: 1;\n}\n\n.col-narration button {\n    display: inline-block;\n}\n\n.col-narration.hide {\n    left: -500px;\n}\n\n.col-content {\n    padding-top: 80px;\n    z-index: 2;\n    left: 250px;\n    right: 0px;\n    overflow: hidden;\n}\n\n.full-wh {\n    width: 100%;\n    height: 100%;\n}\n\n.content-cover {\n    position: absolute;\n    top: 0;\n    right: 0;\n    bottom: 0;\n    left: 0;\n    -webkit-transition: all 0.5s ease;\n            transition: all 0.5s ease;\n    background: #FFF;\n    opacity: 1;\n    background-image: url('/public/css/icons/loading.gif');\n    background-repeat: no-repeat;\n    background-size: 32px 32px;\n    background-position: right 30px bottom 30px;\n}\n\n.content-cover.fade {\n    opacity: 0;\n}\n\n.help-container {\n    position: absolute;\n    bottom: 25px;\n    left: 0;\n    right: 0;\n    font-size: 12px;\n    color: #666;\n    cursor: pointer;\n    text-align: center;\n    text-decoration: underline;\n}\n\n.button-close {\n    font-size: 75%;\n    background: #Ff8c00;\n    color: white;\n}\n", ""]);
...
/***/ function(module, exports, __webpack_require__) {
	...
	if(false) {
		// When the styles change, update the <style> tags
		if(!content.locals) {
			module.hot.accept("!!./../../node_modules/css-loader/index.js!./../../node_modules/cssnext-loader/index.js!./menu.css", function() {
				var newContent = require("!!./../../node_modules/css-loader/index.js!./../../node_modules/cssnext-loader/index.js!./menu.css");
				if(typeof newContent === 'string') newContent = [[module.id, newContent, '']];
				update(newContent);
			});
		}
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports = module.exports = __webpack_require__(210)();
	exports.push([module.id, "\n.col-section ul {\n    list-style: none;\n    margin: 0;\n    padding: 0;\n}\n\n.col-section ul.level-0 {\n    padding-top: 80px;\n}\n\n.col-section ul.level-1 {\n}\n\n.col-section ul.level-1.active {\n}\n\n.col-section .item {\n    cursor: pointer;\n    overflow: hidden;\n    text-overflow: ellipsis;\n    color: #222;\n    white-space: nowrap;\n    position: relative;\n    padding: 0.5em 1em;\n    padding-right: 25px;\n}\n\n.col-section .item > .chevron {\n    background: url('/public/css/icons/chevron-right.svg');\n    background-size: cover;\n    width: 12px;\n    height: 12px;\n    position: absolute;\n    right: 10px;\n    top: 10px;\n    opacity: 0.4;\n    -webkit-transition: all 0.5s ease;\n            transition: all 0.5s ease;\n}\n\n.col-section .item.active > .chevron {\n    -webkit-transform: rotate(90deg);\n            transform: rotate(90deg);\n    opacity: 1;\n}\n\n.col-section .item.level-1 {\n    font-weight: bold;\n    border-top: 1px solid #CCC;\n}\n\n.col-section .item.level-1:last-child {\n    border-bottom: 1px solid #CCC;\n}\n\n.col-section .item.level-2 {\n    background: #FAFAFA;\n}\n\n.col-section .item.selected {\n    color: #FFF;\n    background: #777;\n}", ""]);
...
/***/ function(module, exports, __webpack_require__) {
	...
	if(false) {
		// When the styles change, update the <style> tags
		if(!content.locals) {
			module.hot.accept("!!./../../node_modules/css-loader/index.js!./../../node_modules/cssnext-loader/index.js!./popup.css", function() {
				var newContent = require("!!./../../node_modules/css-loader/index.js!./../../node_modules/cssnext-loader/index.js!./popup.css");
				if(typeof newContent === 'string') newContent = [[module.id, newContent, '']];
				update(newContent);
			});
		}
...
/***/ function(module, exports, __webpack_require__) {
	...
	if(false) {
		// When the styles change, update the <style> tags
		if(!content.locals) {
			module.hot.accept("!!./../../node_modules/css-loader/index.js!./../../node_modules/cssnext-loader/index.js!./narration.css", function() {
				var newContent = require("!!./../../node_modules/css-loader/index.js!./../../node_modules/cssnext-loader/index.js!./narration.css");
				if(typeof newContent === 'string') newContent = [[module.id, newContent, '']];
				update(newContent);
			});
		}
